...
function populateEventTable() {
    ...
    entries.forEach(e => {
        ...
        if (pb && e === pb) tdTime.style.color = "gold"; // ✅ PB highlighting UNCHANGED
...
// ✅ FIXED CSV EXPORT (ONLY CHANGE HERE)
function exportCSV() {
    let entries = [...filteredEntries()].sort((a, b) => {
        if (currentSort.key === "date") return new Date(a.date) - new Date(b.date);
        if (currentSort.key === "time") return timeToSeconds(a.time) - timeToSeconds(b.time);
        if (currentSort.key === "happiness") return (a.happiness || 0) - (b.happiness || 0);
        return 0;
    ...
    let csv = "Distance,Stroke,Course,Date,Time,Happiness,Comments\n";
    ...
    const safeName = `${username}_${distance}_${stroke}_${course}`
        .replace(/\s+/g, "_")
        .replace(/[^\w\-]/g, "");
