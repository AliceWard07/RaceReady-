...
function timeToSeconds(t) {
    ...
    // Colon format (preferred): HH:MM:SS / MM:SS / M:SS.ss
    if (t.includes(":")) {
    ...
    // Dot format with 2 parts → seconds
    if (/^\d+\.\d+$/.test(t)) {
        return parseFloat(t);
    }

    // Dot format with 3 parts → MM.SS.hh (swimming-style)
    if (/^\d+\.\d+\.\d+$/.test(t)) {
...
function renderTimeChart(data) {
    ...
    const values = data.map(e => timeToSeconds(e.time));
    const maxTime = Math.max(...values);

    newChart(ctx, {
        labels: data.map(e => `${e.distance} ${e.stroke}`),
        values: values,
        label: maxTime >= 60 ? "Time (mm:ss)" : "Time (seconds)",
        yFormat: maxTime >= 60
            ? formatTimeMMSS
            : v => v.toFixed(2),
        tooltipFormat: maxTime >= 60
            ? formatTimeMMSS
            : v => v.toFixed(2)
    });
}
